...
LinkIO.prototype.bindEvents = function(monitoring) {
    ...
    io.use(function(socket, next) {
        var checkingUser = usersModel.find({"mail" : socket.handshake.query.mail});
        checkingUser.exec(function (err, users) {
            if (err) {
                throw err;
            }
			if (users.length == 1 && passwordHash.verify(socket.handshake.query.password, users[0].password)) {
				...
				checkingConnection.exec(function (err, applications) {
                    ...
                    if(!apiKeyFound) {
                        that.log.log("[" + socket.handshake.query.mail + "] failed to connect (API key doesn't exit).", Log.TYPE.INFO, Log.LEVEL.DEBUG);
                        next(new Error("API_KEY ERROR"));
                    }
				});
			}
            else {
                var message = users.length == 0 ? "account doesn't exist" : "wrong password"
                that.log.log("[" + socket.handshake.query.mail + "] failed to connect (" + message + ").", Log.TYPE.INFO, Log.LEVEL.DEBUG);
                next(new Error(users.length == 0 ? "ACCOUNT ERROR" : "PASSWORD ERROR"));
            }
        });
    });
    ...
    io.on('connection', function(socket){
        ...
        socket.on('disconnect', function () {
            ...
            that.log.log("[" + socket._client.mail + "] disconnected " + socket.id, Log.TYPE.INFO, Log.LEVEL.DEBUG);
