...
LinkIO.prototype.start = function() {
    ...
    io.on('connection', function(socket){
        ...
        socket.on("joinRoom", function(id, cb) {
        ...
        socket.on("getAllUsers", function(callback) {
            if(typeof user.room != 'undefined') {
                callback(user.room.getAllUsers());
            }else{
                callback([]);
            }
        });
        ...
        socket.on("eventToList", function(e) {
            for (clientID in e.idList) {
                socket.broadcast.to(clientID).emit("eventToList", e);
            }
        });
