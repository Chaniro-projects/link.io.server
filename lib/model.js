...
model.Client.prototype.joinRoom = function(id) {
     ...
     if(typeof g != 'undefined') {
          ...
          model.linkIO.log.log("[" + this.mail + "] joined room [" + g.id + "]", Log.TYPE.INFO, Log.LEVEL.DEBUG);

          return g;
     }
     else {
          return new model.Room(this, this.api_key, id);
     }
}
...
model.Client.prototype.disconnect = function() {
     ...
     for(var i = model.users.length - 1; i>=0; i--) {
          if(model.users[i].socket.id == that.socket.id) {
...
model.Client.getFromSocket = function(socket) {
     model.users.forEach(function(c) {
...
model.Room = function(user, api_key, id) {
     ...
     model.linkIO.log.log("[" + user.mail + "] created and joined room [" + this.id + "]", Log.TYPE.INFO, Log.LEVEL.DEBUG);
}
