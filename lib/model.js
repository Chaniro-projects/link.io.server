 ...
 * @constructor
 */
model.Client = function(l, r, s, rights, api_key, appName) {
     this.socket = s;
     this.login = l;
     this.role = r;
     this.room = undefined;
...
model.Client.prototype.joinRoom = function(id) {
     ...
     if(typeof g != 'undefined') {
          ...
          model.linkIO.log.log("[" + this.login + " - (" + this.role + ")] joined room [" + g.id + "]", Log.TYPE.INFO, Log.LEVEL.DEBUG);

          return g;
     }
     else {
          var r = new model.Room(this, id, this.api_key);
...
model.Room = function(user, api_key, id) {
     ...
     model.linkIO.log.log("[" + user.login + " - (" + user.role + ")] created and joined room [" + this.id + "]", Log.TYPE.INFO, Log.LEVEL.DEBUG);
}
 ...
 * @returns {Array[{login,id}]} Users
 */
model.Room.prototype.getAllUsers = function() {
      var ret = [];
      for(var i = 0; i<this.users.length; i++)
           ret.push({login: this.users[i].login, id:this.users[i].socket.id});
      return ret;
}
...
model.Room.prototype.leave = function(user) {
     ...
     if(this.users.length == 0) {
          for(var j = 0; j<model.applications.length; j++){
               for(var k = 0; i<model.rooms.length; k++) {
